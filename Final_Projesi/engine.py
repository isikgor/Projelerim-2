...
class Engine:
    ...
    def __init__(self,game = str,width = int,height = int, Xpad = int, Ypad = int):
        ...
        pg.display.set_caption("Taş Kağıt Makas")
    ...
    def play_with_friend(self,c = connection.Connection):
        ...
        self.set_text("Bekleniyor " + self.Player2.Name,self.middleX-150,self.middleY+70)
        ...
        self.init()
        self.set_text(self.Player2.Name + ' Seçti: '+ recv[1],self.middleX-150,self.middleY+70)
            ...
            if self.Player1.Selection == self.Player2.Selection:
                ...
                if self.Player1.Selection == "rock":
                    if self.Player2.Selection =="paper":
                        ...
                        return (self.Player2.Name+" kazandı.")
                    else:
                        self.Player1.add_point(1)
                        return (self.Player1.Name+" kazandı")
    ...
    def select(self,player,s_value):
        ...
        if s_value == "rock":
            value = "Seçtiniz: Taş" if player == "p1" else self.Player2.Name + " Seçti: Taş"
            self.set_text(value, self.middleX - 120,self.middleY + 50)
            pg.draw.rect(self.Screen, color, [self.middleX-160,self.middleY-50, 100, 100], 2)

        elif s_value == "paper":
            value = "Seçtiniz: Kağıt" if player == "p1" else self.Player2.Name + " Seçti: Kağıt"
            self.set_text(value,self.middleX-120,self.middleY + 50)
            pg.draw.rect(self.Screen, color , [self.middleX-50,self.middleY-50, 100, 100], 2)

        elif s_value == "scissors":
            value = "Seçtiniz: Makas" if player == "p1" else self.Player2.Name + " Seçti: Makas"
            self.set_text(value,self.middleX-120,self.middleY + 50)
            pg.draw.rect(self.Screen, color , [self.middleX+60,self.middleY-50, 100, 100], 2)
        else:
            raise Exception("Seçim yanlış")
        pg.display.update()
    ...
    def render(self,master,conn = None):
        ...
        if self.Running == False:
            self.set_text("Lütfen seçim yapınız.",self.middleX-150,self.middleY+70)
        self.Running = True
        self.Playable = False
        self.Master = master
        ...
        while self.Running:
            for event in pg.event.get():
                ...
                if event.type == pg.MOUSEBUTTONUP:
                    ...
                    if int(self.ScreenInfo["width"]-40)<=pos[0]<=int(self.ScreenInfo["width"]) and 0 <= pos[1] <= 40:
                        self.set_text("Created by Abulkadir Işıkgör",self.middleX-150,self.middleY+70)
            ...
            if self.Player1.Point >= 5 or self.Player2.Point >= 5:
                messagebox.showinfo("Oyun bitti","Kazanan: "+ (self.Player1.Name if self.Player1.Point > self.Player2.Point else self.Player2.Name ))
...
class Player:
    ...
    def rename(self,new_name):
        self.Name = new_name
    def __str__(self):
        return str.format(" {0} --- Puanı : {1}.",self.Name,self.Point)
    def set_selection(self,value):
        self.Selection = value

    @staticmethod
    def create_computer():
        com = Player("Bilgisayar")
        return com
